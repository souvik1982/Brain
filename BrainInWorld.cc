...
{ 
  ...
  {
    Fire *fire=new Fire(r3->Rndm()*100., r3->Rndm()*100., r3->Rndm()*2.*pi);
    fires.push_back(fire);
  }
  
  typedef std::vector<Food*> Foods;
  Foods foods;
  unsigned int nFoods=10;
  for (unsigned int i=0; i<nFoods; ++i)
  {
    Food *food=new Food(r3->Rndm()*100., r3->Rndm()*100., r3->Rndm()*2.*pi);
  ...
  // std::vector <double> speed_time_vector;
  ...
  while (foods.size()>0 && bots.size()>0)
  {
    ...
    for (unsigned int i=0; i<bots.size(); ++i)
    {
      ...
      if (eatenFood>=0) 
      {
        ...
        if (r3->Rndm()>0.01)
        {
          Food *food=new Food(r3->Rndm()*100., r3->Rndm()*100., r3->Rndm()*pi);
          foods.push_back(food);
        }
        // speed_vector.push_back(bots.at(i)->eaten());
        // speed_time_vector.push_back(time);
